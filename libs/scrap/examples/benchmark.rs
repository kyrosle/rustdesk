...
use scrap::{
  ...
  Capturer, Display, TraitCapturer, VpxDecoder, VpxDecoderConfig, VpxEncoder,
  VpxEncoderConfig,
...
fn test_vpx(
  ...
  let mut decoder =
    VpxDecoder::new(VpxDecoderConfig { codec: codec_id }).unwrap();
  let start = Instant::now();
  for vpx in vpxs {
...
mod hw {
  ...
  pub fn vpx_yuv_to_hw_yuv(
    ...
    let (linesize, offset, length) =
      ffmpeg_linesize_offset_length(pixfmt, width, height, HW_STRIDE_ALIGN)
        .unwrap();
    for mut yuv in yuvs {
      i420_to_bgra(width, height, &yuv, &mut bgra);
      if pixfmt == AVPixelFormat::AV_PIX_FMT_YUV420P {
        hw_bgra_to_i420(
          width, height, &linesize, &offset, length, &bgra, &mut yuv,
        );
      } else {
        hw_bgra_to_nv12(
          width, height, &linesize, &offset, length, &bgra, &mut yuv,
