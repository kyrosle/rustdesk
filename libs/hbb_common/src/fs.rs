...
use crate::{
  anyhow::anyhow, bail, get_version_number, message_proto::*, ResultType,
  Stream,
};
...
impl TransferJob {
  ...
  pub async fn read(
    &mut self,
    stream: &mut Stream,
  ) -> ResultType<Option<FileTransferBlock>> {
    let file_num = self.file_num as usize;
    if file_num >= self.files.len() {
      self.file.take();
      return Ok(None);
  ...
  pub fn job_completed(&self) -> bool {
    ...
    !self.enable_overwrite_detection
      || (!self.file_confirmed && !self.file_is_waiting)
  }
...
#[inline]
pub fn new_error<T: std::string::ToString>(
  id: i32,
  err: T,
  file_num: i32,
) -> Message {
