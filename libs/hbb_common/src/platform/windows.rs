...
use winapi::{
  ...
  um::{
    ...
    pdh::{
      PdhAddCounterA, PdhCloseQuery, PdhCollectQueryData,
      PdhCollectQueryDataEx, PdhGetFormattedCounterValue, PdhOpenQueryA,
...
pub fn start_cpu_performance_monitor() {
  ...
  let f = || unsafe {
    ...
    let event =
      CreateEventA(std::ptr::null_mut(), FALSE, FALSE, std::ptr::null() as _);
    if event.is_null() {
    ...
    loop {
      ...
      if queue.len() > 0
        && recent_valid.iter().filter(|v| **v).count() > queue.len() / 2
      {
