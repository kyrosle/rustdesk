...
impl PipeWireRecorder {
  pub fn new(capturable: PipeWireCapturable) -> Result<Self, Box<dyn Error>> {
    ...
    let appsink = sink.dynamic_cast::<AppSink>().map_err(|_| {
      GStreamerError("Sink element is expected to be an appsink!".into())
    })?;
...
impl Recorder for PipeWireRecorder {
  fn capture(
    &mut self,
    timeout_ms: u64,
  ) -> Result<PixelProvider, Box<dyn Error>> {
      ...
      .try_pull_sample(gst::ClockTime::from_mseconds(timeout_ms))
    {
      ...
      if let Err(..) =
        crate::would_block_if_equal(&mut self.saved_raw_data, buf.as_slice())
      {
...
fn request_screen_cast(
  ...
  handle_response(
    ...
    move |r: OrgFreedesktopPortalRequestResponse, c, _| {
      ...
      let plasma = std::env::var("DESKTOP_SESSION")
        .map_or(false, |s| s.contains("plasma"));
      ...
      let session: dbus::Path = r
        ...
        .as_str()
        .ok_or_else(|| {
          DBusError("Failed to convert session_handle to string.".into())
        })?
        .to_string()
        .into();
