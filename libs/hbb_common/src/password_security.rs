...
pub fn encrypt_str_or_original(
  s: &str,
  version: &str,
  max_len: usize,
) -> String {
...
pub fn decrypt_str_or_original(
  s: &str,
  current_version: &str,
) -> (String, bool, bool) {
...
pub fn encrypt_vec_or_original(
  v: &[u8],
  version: &str,
  max_len: usize,
) -> Vec<u8> {
...
// bool: whether decryption is successful
// bool: whether should store to re-encrypt when load
pub fn decrypt_vec_or_original(
  v: &[u8],
  current_version: &str,
) -> (Vec<u8>, bool, bool) {
...
fn encrypt(v: &[u8], max_len: usize) -> Result<String, ()> {
  if !v.is_empty() && v.len() <= max_len {
    symmetric_crypt(v, true)
      .map(|v| base64::encode(v, base64::Variant::Original))
  } else {
    Err(())
  }
}
...
mod test {
  ...
  fn test() {
    ...
    let test_speed = |len: usize, name: &str| {
      ...
      let start = Instant::now();
      let decrypted =
        base64::decode(&encrypted, base64::Variant::Original).unwrap();
      let t2 = start.elapsed();
      assert_eq!(data, decrypted);
