...
pub fn gen_version() {
  ...
  for line in read_lines("Cargo.toml").unwrap().flatten() {
    ...
    if ab.len() == 2 && ab[0] == "version" {
      file
        .write_all(format!("pub const VERSION: &str = {};\n", ab[1]).as_bytes())
        .ok();
      break;
    }
  }
...
pub fn init_log(
  _is_async: bool,
  _name: &str,
) -> Option<flexi_logger::LoggerHandle> {
...
mod test {
  ...
  fn test_mangle() {
    let addr =
      SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(192, 168, 16, 32), 21116));
  ...
  fn test_mangle2() {
    ...
    assert_eq!(
      AddrMangle::decode(&AddrMangle::encode(
