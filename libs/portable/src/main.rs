...
fn main() {
  ...
  let click_setup =
    args.is_empty() && arg_exe.to_lowercase().ends_with("install.exe");
  let quick_support =
    args.is_empty() && arg_exe.to_lowercase().ends_with("qs.exe");
...
mod windows {
  use std::{
    fs, os::windows::process::CommandExt, path::PathBuf, process::Command,
  };

  // Used for privacy mode(magnifier impl).
  pub const RUNTIME_BROKER_EXE: &'static str =
    "C:\\Windows\\System32\\RuntimeBroker.exe";
  pub const WIN_MAG_INJECTED_PROCESS_EXE: &'static str =
  ...
  pub(super) fn copy_runtime_broker(dir: &PathBuf) {
    ...
    if target_file.exists() {
      if let (Ok(src_file), Ok(tgt_file)) =
        (fs::read(src), fs::read(&target_file))
      {
    ...
    let _allow_err = Command::new("taskkill")
      ...
      .output();
    let _allow_err =
      std::fs::copy(src, &format!("{}\\{}", dir.to_string_lossy(), tgt));
  }
}
