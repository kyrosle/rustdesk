...
use crate::mediacodec::{
  MediaCodecDecoder, MediaCodecDecoders, H264_DECODER_SUPPORT,
  H265_DECODER_SUPPORT,
};
use crate::{
  aom::{self, AomDecoder, AomEncoder, AomEncoderConfig},
  common::GoogleImage,
  vpxcodec::{
    self, VpxDecoder, VpxDecoderConfig, VpxEncoder, VpxEncoderConfig,
    VpxVideoCodecId,
  },
  CodecName, ImageRgb,
};
...
pub trait EncoderApi {
  ...
  fn encode_to_message(&mut self, frame: &[u8], ms: i64)
    -> ResultType<Message>;
...
impl Encoder {
  ...
  pub fn update(id: i32, update: EncodingUpdate) {
    ...
    let vp8_useable =
      decodings.len() > 0 && decodings.iter().all(|(_, s)| s.ability_vp8 > 0);
    let av1_useable =
    ...
    match preference {
      ...
      PreferCodec::H264 => {
        *name = h264_name.map_or(auto_codec, |c| CodecName::H264(c))
      }
      PreferCodec::H265 => {
        *name = h265_name.map_or(auto_codec, |c| CodecName::H265(c))
      }
