...
use winapi::{
  shared::{
    ...
    minwindef::{
      BOOL, DWORD, FALSE, FARPROC, HINSTANCE, HMODULE, HRGN, TRUE, UINT,
    },
    ntdef::{LONG, NULL},
    windef::{HWND, RECT},
  ...
  um::{
    ...
    libloaderapi::{
      FreeLibrary, GetModuleHandleExA, GetProcAddress, LoadLibraryExA,
    },
    winuser::*,
  },
};
...
pub type MagInitializeFunc =
  ::std::option::Option<unsafe extern "C" fn() -> BOOL>;
pub type MagUninitializeFunc =
  ::std::option::Option<unsafe extern "C" fn() -> BOOL>;
...
impl MagInterface {
  fn new() -> Result<Self> {
    ...
    unsafe {
      ...
      s.set_window_filter_list_func = Some(std::mem::transmute(
        Self::load_func(s.lib_handle, "MagSetWindowFilterList")?,
      ));
      s.set_image_scaling_callback_func = Some(std::mem::transmute(
        Self::load_func(s.lib_handle, "MagSetImageScalingCallback")?,
      ));
...
impl CapturerMag {
  ...
  pub(crate) fn new(
    ...
    unsafe {
      ...
      if let Some(set_callback_func) =
        s.mag_interface.set_image_scaling_callback_func
      {
        if FALSE
          == set_callback_func(
            s.magnifier_window,
  ...
  pub(crate) fn frame(&mut self, data: &mut Vec<u8>) -> Result<()> {
    ...
    unsafe {
      ...
      if let Some(set_window_source_func) =
        self.mag_interface.set_window_source_func
      {
