    ...
    #[cfg(target_os = "linux")]
    {
      if _tmp == "/root" {
        if let Ok(output) = std::process::Command::new("whoami").output() {
          ...
          if user != "root" {
            let cmd =
              format!("getent passwd '{}' | awk -F':' '{{print $6}}'", user);
...
#[inline]
pub fn store_path<T: serde::Serialize>(
  path: PathBuf,
  cfg: T,
) -> crate::ResultType<()> {
...
impl Config {
  ...
  fn store(&self) {
    let mut config = self.clone();
    config.password = encrypt_str_or_original(
      &config.password,
      PASSWORD_ENC_VERSION,
      ENCRYPT_MAX_LEN,
    );
    config.enc_id = encrypt_str_or_original(
      &config.id,
      PASSWORD_ENC_VERSION,
      ENCRYPT_MAX_LEN,
    );
  ...
  pub fn ipc_path(postfix: &str) -> String {
    ...
    #[cfg(not(windows))]
    {
      ...
      let mut path: PathBuf =
        format!("{}/{}", *APP_DIR.read().unwrap(), *APP_NAME.read().unwrap())
          .into();
      #[cfg(not(target_os = "android"))]
      let mut path: PathBuf =
        format!("/tmp/{}", *APP_NAME.read().unwrap()).into();
...
impl PeerConfig {
  ...
  fn path(id: &str) -> PathBuf {
    ...
    if let Ok(forbidden_paths) = forbidden_paths {
      let id_encoded = if forbidden_paths.is_match(id) {
        "base64_".to_string()
          + base64::encode(id, base64::Variant::Original).as_str()
      } else {
        id.to_string()
      };
        ...
        .collect::<Result<Vec<_>, _>>()
      {
        let mut peers: Vec<_> = peers
          ...
          .map(|p| {
            ...
            let id_decoded_string =
              if id.starts_with("base64_") && id.len() != 7 {
                let id_decoded =
                  base64::decode(&id[7..], base64::Variant::Original)
                    .unwrap_or_default();
  ...
  fn deserialize_custom_image_quality<'de, D>(
    deserializer: D,
  ...
  where
    D: de::Deserializer<'de>,
  {
    let mut mp: HashMap<String, String> =
