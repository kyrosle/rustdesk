...
impl Distro {
  fn new() -> Self {
    let name = run_cmds("awk -F'=' '/^NAME=/ {print $2}' /etc/os-release")
      ...
      .to_string();
    let version_id =
      run_cmds("awk -F'=' '/^VERSION_ID=/ {print $2}' /etc/os-release")
        .unwrap_or_default()
        .trim()
        .trim_matches('"')
        .to_string();
...
pub fn get_display_server_of_session(session: &str) -> String {
  ...
  // Check session type of the session
  {
    ...
    if display_server == "tty" {
      // If the type is tty...
      if let Ok(output) =
        run_loginctl(Some(vec!["show-session", "-p", "TTY", session]))
      // Get the tty number
      {
        ...
        if let Ok(xorg_results) =
          run_cmds(&format!("ps -e | grep \"{tty}.\\\\+Xorg\""))
...
pub fn is_active(sid: &str) -> bool {
  if let Ok(output) =
    run_loginctl(Some(vec!["show-session", "-p", "State", sid]))
  {
