...
use winapi::{
  ...
  um::{
    d3d11::*, d3dcommon::D3D_DRIVER_TYPE_UNKNOWN, unknwnbase::IUnknown,
...
impl Capturer {
  pub fn new(display: Display, use_yuv: bool) -> io::Result<Capturer> {
    ...
    if res.is_err() {
      ...
      res = wrap_hresult(unsafe {
        let hres = (*display.inner.0)
          .DuplicateOutput(device.0 as *mut _, &mut duplication);
        if hres != S_OK {
          gdi_capturer = display.create_gdi();
          println!("Fallback to GDI");
          if gdi_capturer.is_some() {
  ...
  unsafe fn load_frame(
    &mut self,
    timeout: UINT,
  ) -> io::Result<(*const u8, i32)> {
...
impl Displays {
  ...
  pub fn get_from_gdi() -> Vec<Display> {
    ...
    loop {
      ...
      let ok =
        unsafe { EnumDisplayDevicesW(std::ptr::null(), i, &mut d as _, 0) };
      if ok == FALSE {
        break;
      }
      i += 1;
