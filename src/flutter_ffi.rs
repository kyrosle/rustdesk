...
fn initialize(app_dir: &str) {
  *config::APP_DIR.write().unwrap() = app_dir.to_owned();
  ...
  #[cfg(target_os = "ios")]
  {
    ...
    let _ =
      try_init_from_env(Env::default().filter_or(DEFAULT_FILTER_ENV, "debug"));
  }
}
...
pub fn session_get_toggle_option(
  session_id: SessionID,
  arg: String,
) -> Option<bool> {
  if let Some(session) = SESSIONS.read().unwrap().get(&session_id) {
...
pub fn session_get_toggle_option_sync(
  session_id: SessionID,
  arg: String,
) -> SyncReturn<bool> {
...
pub fn session_toggle_option(session_id: SessionID, value: String) {
  ...
  #[cfg(not(any(target_os = "android", target_os = "ios")))]
  if SESSIONS.read().unwrap().get(&session_id).is_some()
...
pub fn session_get_flutter_option(
  session_id: SessionID,
  k: String,
) -> Option<String> {
  if let Some(session) = SESSIONS.read().unwrap().get(&session_id) {
...
pub fn session_get_flutter_option_by_peer_id(
  id: String,
  k: String,
) -> Option<String> {
  if let Some((_, session)) =
    SESSIONS.read().unwrap().iter().find(|(_, s)| s.id == id)
  {
    Some(session.get_flutter_option(k))
  } else {
    None
...
pub fn session_get_reverse_mouse_wheel(
  session_id: SessionID,
) -> Option<String> {
  if let Some(session) = SESSIONS.read().unwrap().get(&session_id) {
...
pub fn session_get_custom_image_quality(
  session_id: SessionID,
) -> Option<Vec<i32>> {
  if let Some(session) = SESSIONS.read().unwrap().get(&session_id) {
...
pub fn session_is_keyboard_mode_supported(
  session_id: SessionID,
  mode: String,
) -> SyncReturn<bool> {
  if let Some(session) = SESSIONS.read().unwrap().get(&session_id) {
...
pub fn session_elevate_with_logon(
  session_id: SessionID,
  username: String,
  password: String,
) {
...
pub fn main_get_peer_option_sync(
  id: String,
  key: String,
) -> SyncReturn<String> {
...
pub fn main_get_peer_flutter_option_sync(
  id: String,
  k: String,
) -> SyncReturn<String> {
...
pub fn main_set_peer_flutter_option_sync(
  id: String,
  k: String,
  v: String,
) -> SyncReturn<()> {
...
pub fn main_set_peer_option_sync(
  id: String,
  key: String,
  value: String,
) -> SyncReturn<bool> {
...
pub fn main_load_recent_peers_for_ab(filter: String) -> String {
  let id_filters =
    serde_json::from_str::<Vec<String>>(&filter).unwrap_or_default();
...
pub fn session_get_audit_server_sync(
  session_id: SessionID,
  typ: String,
) -> SyncReturn<String> {
...
pub fn plugin_register_event_stream(
  _id: String,
  _event2ui: StreamSink<EventToUI>,
) {
  #[cfg(feature = "plugin_framework")]
  {
    crate::plugin::native_handlers::session::session_register_event_stream(
      _id, _event2ui,
    );
  }
}
...
#[inline]
pub fn plugin_set_session_option(
  _id: String,
  _peer: String,
  _key: String,
  _value: String,
...
#[inline]
pub fn plugin_get_shared_option(
  _id: String,
  _key: String,
) -> SyncReturn<Option<String>> {
  #[cfg(feature = "plugin_framework")]
